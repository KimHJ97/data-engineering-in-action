...
# 토픽 생성
$ kafka-topics.sh --create --bootstrap-server kafka_node1:9092 --replication-factor 1 --partitions 3 --topic news
```

 - API 서버를 호출하여 데이터를 얻고, 해당 데이터를 프로듀서를 통해 토픽에 메시지를 전송한다.
```python
import json
import requests
...
producer = Producer({'bootstrap.servers': 'IP:9092'})
while True:
    api_server='http://API-SERVER-IP:5000/api'
